...
"#,
    ...
    async fn test_get_tx_info() {
        ...
        let tx_data = <Vec<u8> as FromHex>::from_hex(&tx_res.hex).unwrap();
        let tx_exp: Transaction = consensus::deserialize(&tx_data).unwrap();
    ...
    async fn test_scripthash_txs() {
        ...
        let tx_data = <Vec<u8> as FromHex>::from_hex(&get_tx.hex).unwrap();
        let expected_tx: Transaction = consensus::deserialize(&tx_data).unwrap();
